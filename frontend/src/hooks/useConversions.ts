...
export function useConversions(page = 1, limit = 20, status?: string) {
  ...
  const fetchConversions = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof ApiError ? err.message : 'Erreur lors du chargement des conversions'
  ...
  const cancelJob = useCallback(async (id: string): Promise<ConversionJob> => {
    try {
      const cancelledJob = await conversionsApi.cancel(id)
      await fetchConversions() // Rafraîchir la liste
      return cancelledJob
    } catch (err) {
      const errorMessage = err instanceof ApiError ? err.message : 'Erreur lors de l\'annulation du job'
      throw new Error(errorMessage)
    }
  }, [fetchConversions])

  const retryJob = useCallback(async (id: string): Promise<ConversionJob> => {
    try {
      const retriedJob = await conversionsApi.retry(id)
      await fetchConversions() // Rafraîchir la liste
      return retriedJob
    } catch (err) {
      const errorMessage = err instanceof ApiError ? err.message : 'Erreur lors de la relance du job'
      throw new Error(errorMessage)
    }
  }, [fetchConversions])
...
export function useJobProgress(jobId: string | null, pollingInterval = 2000) {
  ...
  const fetchProgress = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof ApiError ? err.message : 'Erreur lors du chargement de la progression'
      setError(errorMessage)
